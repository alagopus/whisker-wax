...
function tags (input) {
  ...
  do {
    ...
    if (last < match.index) {
      result.push(['write', inputString.substring(last, match.index)])
    }
    if (match[1]) {
  ...
  while (index < result.length) {
    ...
    switch (match && match[1]) {
      ...
      case '/':
        ...
        result[begin] = [ match[1] === '^' ? 'not' : 'map', start, block ]
        index = begin + 1
        break
      case '&':
        result[index++] = [ 'print', match.input.substring(match[0].length) ]
        break
      case '>':
        result[index++] = [ 'part', match.input.substring(match[0].length) ]
        break
      case '=':
