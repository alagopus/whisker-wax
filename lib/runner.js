...
function next (scan, name) {
  if (scan) {
    return (typeof scan.get === 'function')
      ? scan.get(name)
      : scan[name]
  }
  return ''
...
Runner.executor = function (template, resolver) {
  return function executor (context) {
    let buffer = ''
    const runner = new Runner({
      write: function (chunk) { buffer += chunk }
    })
...
Runner.resolver = function (_require) {
  return function resolver (filename) {
    const executor = _require(filename)
    return {
...
Runner.create = (template, _require) => {
  const resolver = Runner.resolver(_require)
  const executor = Runner.executor(template, resolver)
